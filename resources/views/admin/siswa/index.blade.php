<x-app-layout>
    ...
    <div x-data="{ 
        ...
        <!-- Filter & Search Bar Dalam 1 Baris Sejajar -->
        <form method="GET" action="{{ route('admin.siswa.index') }}" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-xs flex flex-col sm:flex-row gap-3 justify-between items-center">
                ...
                <table class="w-full text-left text-xs text-gray-600">
                    <thead class="bg-gray-50/75 text-gray-400 uppercase font-bold text-[10px] tracking-wider border-b border-gray-100">
                        <tr>
                            ...
                            <!-- Header Sorting: Nama Siswa -->
                            <th class="px-4 py-3">
                                @php
                                    $nextDirection = (request('sort_by') == 'nama_siswa' && request('sort_direction') == 'asc') ? 'desc' : 'asc';
                                @endphp
                                <a href="{{ route('admin.siswa.index', array_merge(request()->query(), ['sort_by' => 'nama_siswa', 'sort_direction' => $nextDirection])) }}" 
                                class="inline-flex items-center gap-1 hover:text-indigo-600 transition group">
                                    <span>SISWA & L/P</span>
                                    <span class="text-gray-400 group-hover:text-indigo-600">
                                        @if(request('sort_by') == 'nama_siswa')
                                            @if(request('sort_direction') == 'asc')
                                                <!-- Arrow Up -->
                                                <svg class="w-3.5 h-3.5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                                            @else
                                                <!-- Arrow Down -->
                                                <svg class="w-3.5 h-3.5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                            @endif
                                        @else
                                            <!-- Arrow Default (Unsorted) -->
                                            <svg class="w-3.5 h-3.5 opacity-40 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg>
                                        @endif
                                    </span>
                                </a>
                            </th>
                            ...
                            <!-- Header Sorting: Kelas -->
                            <th class="px-4 py-3">
                                @php
                                    $nextDirectionKelas = (request('sort_by') == 'kelas' && request('sort_direction') == 'asc') ? 'desc' : 'asc';
                                @endphp
                                <a href="{{ route('admin.siswa.index', array_merge(request()->query(), ['sort_by' => 'kelas', 'sort_direction' => $nextDirectionKelas])) }}" 
                                class="inline-flex items-center gap-1 hover:text-indigo-600 transition group">
                                    <span>KELAS</span>
                                    <span class="text-gray-400 group-hover:text-indigo-600">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-xs text-gray-500 leading-relaxed">
                                    Apakah Anda yakin ingin menghapus data siswa <span class="font-bold text-gray-800" x-text="deleteItemName"></span>? Tindakan ini tidak dapat dibatalkan dan akan menghapus seluruh data akademik siswa tersebut.
                                </p>
                            </div>
                        </div>
                    </div>
