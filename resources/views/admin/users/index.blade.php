<x-app-layout>
    ...
    <div x-data="{ 
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-3.5 px-5">
                                        <form action="{{ route('admin.users.update-role', $user->id) }}" method="POST" class="flex items-center gap-2">
                                            ...
                                            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-semibold px-2.5 py-1 rounded-lg transition">
                                                Simpan
                                            </button>
                                        </form>
                                    </td>
        ...
        <!-- ========================================== -->
        <!-- AREA MODAL (TELEPORTED / DILUAR CONTAINER) -->
        <!-- ========================================== -->
        <template x-teleport="body">
            <div>
                <!-- MODAL TAMBAH USER -->
                <div x-cloak x-show="modalTambah" class="fixed inset-0 z-[999] overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
                    <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl" @click.away="modalTambah = false">
                        <div class="flex justify-between items-center border-b pb-3">
                            ...
                            <button @click="modalTambah = false" class="text-gray-400 hover:text-gray-600 text-lg leading-none">&times;</button>
                        </div>
                        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-3">
                            @csrf
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Nama Lengkap</label>
                                <input type="text" name="name" required class="w-full text-xs border-gray-200 rounded-xl bg-gray-50/50 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Email</label>
                                <input type="email" name="email" required class="w-full text-xs border-gray-200 rounded-xl bg-gray-50/50 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Password</label>
                                <input type="password" name="password" autocomplete="new-password" required class="w-full text-xs border-gray-200 rounded-xl bg-gray-50/50 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Role Awal</label>
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($roles as $r)
                                        <label class="inline-flex items-center gap-1.5 text-xs text-gray-600 bg-gray-50 border border-gray-200 p-2 rounded-xl cursor-pointer">
                                            <input type="checkbox" name="roles[]" value="{{ $r->name }}" class="w-3.5 h-3.5 text-indigo-600 rounded focus:ring-indigo-500">
                            ...
                            <div class="flex justify-end gap-2 pt-3 border-t border-gray-100">
                                <button type="button" @click="modalTambah = false" class="px-4 py-2 text-xs font-semibold bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-600 transition">Batal</button>
                                <button type="submit" class="px-4 py-2 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition">Simpan User</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <!-- MODAL EDIT USER -->
                <div x-cloak x-show="modalEdit" class="fixed inset-0 z-[999] overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
                    <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl" @click.away="modalEdit = false">
                        <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                            ...
                            <button @click="modalEdit = false" class="text-gray-400 hover:text-gray-600 text-lg leading-none">&times;</button>
                        </div>
                        <form :action="editActionUrl" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Nama Lengkap</label>
                                <input type="text" name="name" x-model="editUser.name" required class="w-full text-xs border-gray-200 rounded-xl bg-gray-50/50 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">Email</label>
                                <input type="email" name="email" x-model="editUser.email" required class="w-full text-xs border-gray-200 rounded-xl bg-gray-50/50 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1">
                                    Password Baru 
                                    <span class="text-[10px] text-gray-400 font-normal ml-1">(Kosongkan jika tidak diganti)</span>
                                </label>
                                <input type="password" 
                                       name="password" 
                                       x-ref="passwordInput"
                            ...
                            <div class="flex justify-end gap-2 pt-3 border-t border-gray-100">
                                <button type="button" @click="modalEdit = false" class="px-4 py-2 text-xs font-semibold bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-600 transition">Batal</button>
                                <button type="submit" class="px-4 py-2 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-sm transition">Update User</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <!-- MODAL DELETE USER -->
                <div x-cloak x-show="modalDelete" class="fixed inset-0 z-[999] overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
                    <div class="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-xl text-center" @click.away="modalDelete = false">
                        ...
                        <form :action="deleteActionUrl" method="POST" class="flex justify-center gap-2 pt-2">
                            @csrf
                            @method('DELETE')
                            <button type="button" @click="modalDelete = false" class="px-4 py-2 text-xs font-semibold bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-600 transition">Batal</button>
                            <button type="submit" class="px-4 py-2 text-xs font-semibold bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-sm transition">Ya, Hapus</button>
                        </form>
                    </div>
                </div>
            </div>
