<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Kelola dan catat kehadiran siswa per sesi mata pelajaran</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('error'))
                <div class="flex items-center p-4 text-rose-800 bg-rose-50 rounded-xl border border-rose-200 shadow-sm" role="alert">
                    <svg class="flex-shrink-0 w-5 h-5 me-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 transition-all hover:shadow-md">
                <form action="{{ route('guru.absensi.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    
                    <div class="md:col-span-3">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Tanggal Sesi</label>
                    ...
                    <div class="md:col-span-3">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Pilih Kelas</label>
                        <select name="kelas_id" class="w-full text-sm border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500 bg-gray-50/50 focus:bg-white transition-all py-2.5" required>
                    ...
                    <div class="md:col-span-4">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Mata Pelajaran</label>
                        <select name="mapel_id" class="w-full text-sm border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500 bg-gray-50/50 focus:bg-white transition-all py-2.5" required>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <form action="{{ route('guru.absensi.store') }}" method="POST">
                                        ...
                                        <tr class="hover:bg-indigo-50/30 transition-colors">
                                            ...
                                            <td class="py-4 px-6">
                                                <div class="flex items-center justify-center gap-2">
                                                    <!-- Option Hadir -->
                                                    <div>
                                                        <input type="radio" id="hadir-{{ $siswa->id }}" name="absensi[{{ $siswa->id }}]" value="Hadir" {{ $currentStatus === 'Hadir' ? 'checked' : '' }} class="peer hidden">
                        ...
                        <div class="p-4 bg-gray-50/80 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <button type="submit" class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold py-2.5 px-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-150 flex items-center justify-center gap-2 text-sm">
